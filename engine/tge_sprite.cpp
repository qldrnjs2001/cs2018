...
namespace tge_sprite {
	...
	{
		putSprite(posx, posy, pObj->m_header.m_cdWorkSpr.X, pObj->m_header.m_cdWorkSpr.Y, g_chiBuffer, pObj->m_pSpriteBuf);
		return 0;
	}
	int put(S_SPRITE_OBJECT *pObj, int posx, int posy, CHAR_INFO *pDestScreenBuf)
	{
		putSprite(posx, posy, pObj->m_header.m_cdWorkSpr.X, pObj->m_header.m_cdWorkSpr.Y, pDestScreenBuf, pObj->m_pSpriteBuf);
		return 0;
	}
	int get(S_SPRITE_OBJECT *pObj, int _xpos, int _ypos, int _width, int _height)
	{
		...
		for (_desy = 0; _desy < _height; _desy++)
		{
			...
			{
				pObj->m_pSpriteBuf[_desx + (_desy*_width)] = g_chiBuffer[(_desx + _xpos) + ((_desy + _ypos)*SCREEN_WIDTH)];
			}
		}
		return 0;
	}
